...
uses
  ...
  function CompletarString(pCadena: string; pCantidad: Integer; pCarcarter: Char): string;
  {$ENDREGION}
...
{$REGION 'procedure Split(Delimiter: Char; Str: string; ListOfStrings: TStrings)'}
procedure Split(Delimiter: Char; Str: string; ListOfStrings: TStrings);
begin
  ListOfStrings.Clear;
...
{$REGION 'function CompletarString(pCadena: string; pCantidad: Integer; pCarcarter: Char): string'}
function CompletarString(pCadena: string; pCantidad: Integer; pCarcarter: Char): string;
var
  mCadena: string;
  mResultado: string;
...
begin
  ...
  mLen := Length(mCadena);
  mLen := pCantidad - mLen;
  for Aux01 := 1 to mLen do
    mResultado := pCarcarter + mResultado;
  mResultado := mResultado + mCadena;
