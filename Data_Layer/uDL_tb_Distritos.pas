unit uDL_tb_Distritos;

interface
uses udmObjects, FireDAC.Comp.Client, System.SysUtils, Data.DB,
  System.Classes, udmConnection, Vcl.ComCtrls;

type
  TDatasetMem = TFDMemTable;

  TDL_tb_Distritos = class
    Dataset: TDatasetMem;
    _ListaCampos: TStringList;
  const
    Tabla = 'tb_Distritos';
    ClaseNombre = 'TDL_tb_Distritos';
  public
    {$REGION 'Metodos'}
    constructor Create;
    destructor Destroy;
...
{$REGION 'Metodos'}
constructor TDL_tb_Distritos.Create;
begin
  Dataset := TDatasetMem.Create(nil);
  with Dataset do
  begin
    FileName := 'tb_Distritos';
    FieldDefs.Clear;
    FieldDefs.Add('Id', ftInteger, 0);
    FieldDefs.Add('Codigo', ftString, 5);
    FieldDefs.Add('Nombre', ftMemo, 0);
    FieldDefs.Add('CodigoProvincia', ftInteger, 0);
    FieldDefs.Add('CodigoCanton', ftInteger, 0);
    CreateDataSet;
    Open;
  end;
...
destructor TDL_tb_Distritos.Destroy;
begin
  Dataset.Close;
  Dataset.Free;
  _ListaCampos.Free;
...
procedure TDL_tb_Distritos.Consultar(var pResultado: Integer;
...
begin
  ...
  if Trim(pSelect) = '' then
    dmObjects.DB_tb_Distritos_Consultar(dmObjects.FDQuery1, Dataset,
...
procedure TDL_tb_Distritos.Insertar(var pResultado: Integer;
...
begin
  ...
  dmObjects.DB_tb_Distritos_Insertar(dmObjects.FDQuery1, Dataset, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_tb_Distritos.Modificar(pWhere: string; var pResultado: Integer;
  var pErrorM: string; FTypeConection: TTypeConection);
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_tb_Distritos_Modificar(dmObjects.FDQuery1, Dataset, mWhere,
    ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_tb_Distritos.Eliminar(pWhere: string; var pResultado: Integer;
  var pErrorM: string; FTypeConection: TTypeConection);
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_tb_Distritos_Eliminar(dmObjects.FDQuery1, mWhere, Tabla, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_tb_Distritos.Limpiar_Tag;
begin
  dmObjects.DB_Limpiar_Tag(Dataset);
end;

function TDL_tb_Distritos.Existe_Id(pWhere: string; var pResultado: Integer;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_tb_Distritos_Existe_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_tb_Distritos.Obtener_Id(pWhere: string; var pResultado: Integer;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_tb_Distritos_Obtener_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_tb_Distritos.Existe_Campo(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_tb_Distritos_Existe_Campo(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

function TDL_tb_Distritos.Obtener_Valor(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_tb_Distritos_Obtener_Valor(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_tb_Distritos.Actualizar_Campo(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_tb_Distritos_Actualizar_Campo(dmObjects.FDQuery1, mWhere, Tabla,
